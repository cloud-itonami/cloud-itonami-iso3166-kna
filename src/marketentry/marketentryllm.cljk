(ns marketentry.marketentryllm
  ...
  the Saint Kitts and Nevis (KNA) public-sector market-entry
  compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "告知期間=" (:notice-published-date e) "->" (:bid-close-date e)
                        " 新聞掲載数=" (:notice-newspaper-count e)
                        " business-licence-verified?=" (:business-licence-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-business-licence? e))
                              (:business-licence-verified? e)))
                   0.9 0.3)}))
