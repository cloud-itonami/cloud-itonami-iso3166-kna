...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (KNA, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up notice-period-insufficient) ==")
    ...
    (println "== filing/submit eng-4 (notice-period-insufficient -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up business-licence-unverified) ==")
    ...
    (println "== filing/submit eng-5 (business-licence-unverified -> HARD hold) ==")
