...
(deftest notice-period-insufficient-is-held-and-unoverridable
  (testing "tender-solicitation notice window falls short of s.13's 42-day/2-newspaper minimum -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:notice-period-insufficient} (-> (store/ledger db) last :basis)))
...
(deftest business-licence-unverified-is-held-and-unoverridable
  (testing "unverified mandatory Business Licence (Cap. 18.20) when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:business-licence-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
